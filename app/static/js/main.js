...
function checkLogin() {
    ...
    if (document.getElementById("loginMail").value == "" || document.getElementById("loginLoz").value == "") {
        error = true;
        var errReq = document.createElement("span");
        errReq.innerHTML = "Podaci moraju biti popunjeni.";

        errMsg.appendChild(errReq);
    } else {
        if (!regMail.test(document.getElementById("loginMail").value)) {
            error = true;
            var errReq = document.createElement("span");
            errReq.innerHTML = "Neispravan format email adrese.";

            errMsg.appendChild(errReq);
        }

        if (document.getElementById("loginLoz").value.length < 8) {
            error = true;
            var errReq = document.createElement("span");
            errReq.innerHTML = "Lozinka mora sadrzavati barem 8 znakova.";
...
function checkPswd() {
    ...
    if (pswd.length < 8) {
        ...
        errPass.innerHTML = "Lozinka mora sadrzavati barem 8 znakova.";
